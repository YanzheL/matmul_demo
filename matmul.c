...
void mm2_v(const double *a, const double *b, double *c, int n1, int n2, int n3) {
  ...
  for (i = 0; i < n1; ++i)
    for (j = 0; j < n3; ++j) {
      register double t = 0;
      for (k = 0; k < n2; k += 8) {
        vector8 va = {a[i*n1 + k], a[i*n1 + k + 1], a[i*n1 + k + 2], a[i*n1 + k + 3], a[i*n1 + k + 4], a[i*n1 + k + 5],
                      a[i*n1 + k + 6], a[i*n1 + k + 7]};
        vector8 vb =
            {b[k*n1 + j], b[(k + 1)*n1 + j], b[(k + 2)*n1 + j], b[(k + 3)*n1 + j], b[(k + 4)*n1 + j], b[(k + 5)*n1 + j],
  ...
  for (i = 0; i < n1; i += 2) {
    for (j = 0; j < n3; j += 2) {
      for (k = 0; k < n2; k += 2) {
        ...
        c[(i + 1)*n1 + j + 1] += a[(i + 1)*n1 + k]*b[k*n1 + j + 1] + a[(i + 1)*n1 + k + 1]*b[(k + 1)*n1 + j + 1];
      }
    }
  }
}
